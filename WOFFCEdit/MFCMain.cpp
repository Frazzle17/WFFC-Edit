...
int MFCMain::Run()
{
	...
	while (WM_QUIT != msg.message)
	{
		...
		{	
			...
			std::wstring statusAppend = L"None";
			if (!IDs.empty())
			{
				statusAppend = L"";
				for (int i = 0; i < IDs.size(); i++)
				{
					statusAppend = statusAppend + std::to_wstring(IDs[i]);
					if (i != IDs.size() - 1)
					{
						statusAppend = statusAppend + L", ";
					}
				}
			}
			std::wstring statusString = L"Selected Object: " + statusAppend;
